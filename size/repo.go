...
func (s *Mongo) CreateSize(value *api.Size) error {
...
func (s *Mongo) UpdateSize(value *api.Size) error {
	body, err := json.Marshal(value)
	if err != nil {
		return err
	}
...
func (s *Mongo) QuerySize(id string) (*api.Size, error) {
...
func (s *Mongo) QuerySizes(where map[string]interface{}, page int, perPage int, sort map[string]interface{}) ([]api.Size, int64, error) {
	items, total, err := s.storage.QueryStorageV3(Schema, where, perPage, page * perPage, sort)
	if err != nil {
		return nil, 0, err
	}
	values := make([]api.Size, 0, len(items))
	for i := range items {
		buf := items[i]
		value := api.Size{}
		err = json.Unmarshal(buf, &value)
